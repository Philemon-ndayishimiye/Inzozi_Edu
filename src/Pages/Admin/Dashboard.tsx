  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard label="Open classes" value={`${openSpots} / ${spots.length}`} sub="Spots currently open" />
        <StatCard label="Applications this month" value={applicationsThisMonth} sub="Submitted via Inzozi" />
        <StatCard label="Pending review" value={pendingReview} sub="Awaiting a decision" />
        <StatCard
          label="Admission Manager"
          value={admissionManagers[0] ? admissionManagers[0].email : 'Not assigned'}
          sub={admissionManagers[0] ? 'Reviewing applications' : 'You\'re reviewing applications'}
        />
      </div>
      ...
      >
        {applicationsLoading ? (
          <div className="px-5 py-10 text-center text-[13px] text-gray-500">Loading…</div>
        ) : applications.length === 0 ? (
          <div className="px-5 py-10 text-center text-[13px] text-gray-500">
            No applications yet. Once a parent applies to one of your open classes, it will show up here.
          </div>
        ) : (
            ...
            <table className="w-full min-w-[480px]">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="text-left font-mono text-[10px] uppercase text-gray-400 px-5 py-2.5">Student</th>
