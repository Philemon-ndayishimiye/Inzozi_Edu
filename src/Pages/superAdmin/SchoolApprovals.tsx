      ...
      <Panel noBodyPadding>
        <div className="p-4">
          <input
            ...
            placeholder={t('schoolApprovalsPage.searchPlaceholder')}
        ...
        {filtered.length === 0 ? (
          <div className="py-14 text-center text-[13.5px] text-gray-500 px-4">
            {isLoading ? t('schoolApprovalsPage.loading') : pending.length === 0 ? t('schoolApprovalsPage.noneWaiting') : t('schoolApprovalsPage.noMatches')}
          </div>
        ) : (
            ...
            <table className="w-full min-w-[520px]">
              <thead>
                <tr className="border-t border-b border-gray-200">
                  <th className="text-left font-mono text-[10px] uppercase text-gray-400 px-4 py-2.5">{t('schoolApprovalsPage.school')}</th>
                  <th className="text-left font-mono text-[10px] uppercase text-gray-400 px-3 py-2.5">{t('schoolApprovalsPage.submittedBy')}</th>
                  <th className="text-left font-mono text-[10px] uppercase text-gray-400 px-3 py-2.5">{t('schoolApprovalsPage.location')}</th>
                ...
                {filtered.map((school) => (
                  <tr key={school.id} className="border-b border-gray-100 last:border-0">
                    <td className="px-4 py-3">
                      ...
                      <div className="text-[11px] text-gray-400">{t('schoolApprovalsPage.newRegistration')}</div>
                    </td>
                    <td className="px-3 py-3 text-[13px]">
                      {school.SchoolManager
                        ? `${school.SchoolManager.firstName} ${school.SchoolManager.lastName}`
                        : <span className="text-gray-400 italic">{t('schoolApprovalsPage.accountDeleted')}</span>}
                    </td>
                      ...
                      <div className="flex items-center gap-2">
                        {school.licenseDocument && (
                          <a
                            ...
                            aria-label={t('schoolApprovalsPage.viewLegalDocument')}
                          >
